...
![image](https://github.com/flioletty/highload/assets/92665311/0dd229c2-6eae-4b11-bc89-25cf3241e537)
...
Основной функционал почты это отправка и получение писем, за которые отвечает сервис `letters`, если падает любой другой, пользователь особо не заметит, а поскольку отправить половину письма или выдавать на чтение каждое третье письмо не представляется возможным, Graceful degradation в почте невозможен в реализации
