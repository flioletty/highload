...
Для обеспечения отказоустойчивости в случае падения одной из нод, будем использовать программное обеспечение keepalived с использованием протокола VRRP. Это позволит мониторить доступность нод, а в случае отказа одной из них перенаправить трафик на другую ноду.

Для проверки работоспособности сервера можем использовать readiness и liveness пробы. С помощью readiness будем проверять готовность сервера принимать трафик. С помощью liveness будем проверять, работает ли сервис.

На уровне L4 можно использовать nginx, который позволит распределить трафик между несколькими серверами.

На уровне L7 будем использовать балансировщик Envoy, так как он быстрее работает с HTTPS и обрабатывает больше запросов

Для шифрования будем использовать Let`s Encrypt. Это автоматизированный открытый центр сертификации. Так же будем использовать Session Cache для улучшения производительности и ускорения подключения.
