...
- Если отправитель является государственным сервисом (например, Госуслуги, Росреестр, ФНС и др.), письмо помещается в специальную папку “Госписьма”.
2. Группировка по номеру обращения:
- Специальный алгоритм собирает уведомления по номеру обращения. Например, если пользователь подал документы на получение загранпаспорта, все обновления статуса будут автоматически приходить в соответствующую цепочку писем — от сообщения о том, что заявление принято, до приглашения на получение.
3. Отметка специальным значком:
- Письма, связанные с государственными услугами, помечаются специальным значком. Это позволяет пользователям быстро увидеть статусы по заявлениям и услугам.
...
- Оценка структуры письма (много ссылок, большой объем текста, картинки).
