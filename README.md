...
![image](https://github.com/flioletty/highload/assets/92665311/8921a122-6a55-47ad-a6e8-e1bfd06cf405)
...
### Алгоритм умной сортировки писем 

1. Анализ отправителя:
- Система анализирует информацию об отправителе письма.
- Если отправитель является государственным сервисом (например, Госуслуги, Росреестр, ФНС и др.), письмо помещается в специальную умную папку “Госписьма”.
2. Группировка по номеру обращения:
- Специальный алгоритм собирает уведомления по номеру обращения.
- Например, если пользователь подал документы на получение загранпаспорта, все обновления статуса будут автоматически приходить в соответствующую цепочку писем — от сообщения о том, что заявление принято, до приглашения на получение.
3. Отметка специальным значком:
- Письма, связанные с государственными услугами, помечаются специальным значком.
- Это позволяет пользователям быстро увидеть статусы по заявлениям и услугам.

### Алгоритм антиспама:

1. Анализ отправителя:
- Проверка отправителя на наличие в базе известных спамеров.
- Отслеживание частоты отправки писем от одного отправителя.
2. Анализ содержания письма:
- Проверка наличия ключевых слов, характерных для спама (например, “победитель”, “бесплатно”, “скидка” и др.).
- Оценка структуры письма (например, много ссылок, большой объем текста).
3. Сравнение с известными шаблонами спама:
- Сравнение содержания письма с известными шаблонами спама.
- Использование машинного обучения для обнаружения новых шаблонов.
4. Отслеживание активности пользователей:
- Анализ частоты отправки писем, активности на сайте и других параметров.
- Блокировка аккаунтов, которые подозрительно активны.
5. Обратная связь от пользователей:
- Пользователи могут помечать письма как спам.
- Использование этой обратной связи для улучшения алгоритма.
